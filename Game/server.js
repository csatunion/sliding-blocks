...
io.sockets.on('connection', function(socket){
	...
	socket.on("game", function(){
		...
		socket.on("currentBlocks", function(blocks, channel){
			socket.broadcast.to(channel).emit("currentBlocks", blocks);
		});
		
		socket.on("toggleScreenShot", function(channel){
			socket.broadcast.to(channel).emit("toggleSendScreenShot");
		});
		
		socket.on("sendUpdate", function(index, xpos, ypos, channel){
			socket.broadcast.to(channel).emit("updateScreen", xpos, ypos, index);
		});
    	...
    	socket.on("nextLevel", function(levelNo, gameid, playerNumber, channel){
    		...
    		fs.readFile(level, 'ascii', function(err, data) {
				...
				//if all levels not complete
					...
					if(playerNumber == 1){
						socket.to(channel).emit("advance", map1, map2, bg);
						socket.broadcast.to(channel).emit("advance", map2, map1, bg);
					} 
					else{
						socket.broadcast.to(channel).emit("advance", map1, map2, bg);
						socket.to(channel).emit("advance", map2, map1, bg);
					}
				}
			});
		});
